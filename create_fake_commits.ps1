# PowerShell script to create 412 fake commits
# Run: .\create_fake_commits.ps1

$ErrorActionPreference = "Stop"
$PROJECT_DIR = "f:\WORK\HydroDraft"
Set-Location $PROJECT_DIR

# Dates
$START_DATE = Get-Date "2026-01-28 08:30:00"
$END_DATE = Get-Date "2026-02-04 22:30:00"

# 412 commit messages in Vietnamese
$COMMITS = @(
...
Write-Host "🚀 TẠO 412 FAKE COMMITS CHO DỰ ÁN HYDRODRAFT" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host "📅 Thời gian: 28/01/2026 - 04/02/2026" -ForegroundColor Yellow
Write-Host "📊 Tổng số commits: $($COMMITS.Count)" -ForegroundColor Yellow
Write-Host "================================================================" -ForegroundColor Cyan

# Init git
Write-Host "`n🔄 Đang khởi tạo Git repository..." -ForegroundColor Green
git init
git config user.email "developer@hydrodraft.vn"
git config user.name "HydroDraft Developer"

# Calculate time interval
$totalSeconds = ($END_DATE - $START_DATE).TotalSeconds
$interval = $totalSeconds / $COMMITS.Count

Write-Host "`n📝 Đang tạo $($COMMITS.Count) commits..." -ForegroundColor Green

for ($i = 0; $i -lt $COMMITS.Count; $i++) {
    $message = $COMMITS[$i]
    
    # Calculate commit time
    $offsetSeconds = Get-Random -Minimum -1800 -Maximum 1800
    $commitTime = $START_DATE.AddSeconds($interval * $i + $offsetSeconds)
    
    # Ensure working hours (8-23)
    if ($commitTime.Hour -lt 8) {
        $commitTime = $commitTime.AddHours(8 - $commitTime.Hour + (Get-Random -Minimum 0 -Maximum 2))
    } elseif ($commitTime.Hour -gt 23) {
        $commitTime = $commitTime.AddHours(-($commitTime.Hour - 22))
    }
    
    $dateStr = $commitTime.ToString("yyyy-MM-ddTHH:mm:ss+07:00")
    
    # Create/update marker file
    $markerContent = "Commit $($i + 1): $message`nTime: $dateStr"
    Set-Content -Path ".progress" -Value $markerContent -Encoding UTF8
    ...
    # Progress
    if ((($i + 1) % 50 -eq 0) -or ($i -eq $COMMITS.Count - 1)) {
        $percent = [math]::Round((($i + 1) / $COMMITS.Count) * 100)
        Write-Host "  ✅ Đã tạo $($i + 1)/$($COMMITS.Count) commits ($percent%)" -ForegroundColor Yellow
    }
}
...
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host "✅ Đã tạo $($COMMITS.Count) commits" -ForegroundColor Yellow
Write-Host "📅 Từ: $($START_DATE.ToString('dd/MM/yyyy HH:mm'))" -ForegroundColor Yellow
Write-Host "📅 Đến: $($END_DATE.ToString('dd/MM/yyyy HH:mm'))" -ForegroundColor Yellow
Write-Host "`n📋 Để xem commits: git log --oneline -20" -ForegroundColor White
Write-Host "⚠️  Để push: git push -f origin main" -ForegroundColor Red
Write-Host "================================================================" -ForegroundColor Cyan
